...
import gettext
t = gettext.translation('config', localedir=os.path.join(FNAMES.Ortho4XP_dir, 'locales'), languages=['en'])
#t.install()
_ = t.gettext

cfg_vars={
    # App
    'verbosity':             {'module':'UI','type':int,'default':1,'values':(0,1,2,3),'hint': _('verbosity hint')},
    'cleaning_level':        {'module':'UI','type':int,'default':1,'values':(0,1,2,3),'hint': _('cleaning_level hint')},
    'overpass_server_choice':{'module':'OSM','type':str,'default':'random','values':['random']+sorted(OSM.overpass_servers.keys()),'hint': _('overpass_server_choice hint')},
    'skip_downloads':        {'module':'TILE','type':bool,'default':False,'hint': _('skip_downloads hint')},
    'skip_converts':         {'module':'TILE','type':bool,'default':False,'hint': _('skip_converts hint')},
    'max_convert_slots':     {'module':'TILE','type':int,'default':4,'values':(1,2,3,4,5,6,7,8),'hint': _('max_convert_slots hint')},
    'check_tms_response':    {'module':'IMG','type':bool,'default':True,'hint': _('check_tms_response hint')},
    'http_timeout':          {'module':'IMG','type':float,'default':10,'hint': _('http_timeout hint')},
    'max_connect_retries':   {'module':'IMG','type':int,'default':5,'hint': _('max_connect_retries hint')},
    'max_baddata_retries':   {'module':'IMG','type':int,'default':5,'hint': _('max_baddata_retries hint')},
    'ovl_exclude_pol'    :   {'module':'OVL','type':list,'default':[0],'hint': _('ovl_exclude_pol hint')},
    'ovl_exclude_net'    :   {'module':'OVL','type':list,'default':[],'hint': _('ovl_exclude_net hint')},
    'custom_scenery_dir':    {'type':str,'default':'','hint': _('custom_scenery_dir hint')},
    'custom_overlay_src':    {'module':'OVL','type':str,'default':'','hint': _('custom_overlay_src hint')},
    # Vector
    'apt_smoothing_pix':   {'type':int,  'default':8,'hint': _('apt_smoothing_pix hint')},
    'road_level':          {'type':int,'default':1,'values':(0,1,2,3,4,5),'hint': _('road_level hint')},
    'road_banking_limit':  {'type':float,'default':0.5,'hint': _('road_banking_limit hint')},
    'lane_width':          {'type':float,'default':5,'hint': _('lane_width hint')},
    'max_levelled_segs':   {'type':int,'default':100000,'hint': _('max_levelled_segs hint')},
    'water_simplification':{'type':float,'default':0,'hint': _('water_simplification hint')},
    'min_area':            {'type':float,'default':0.001,'hint': _('min_area hint')},
    'max_area':            {'type':float,'default':200,'hint': _('max_area hint')},
    'clean_bad_geometries':{'type':bool,'default':True,'hint': _('clean_bad_geometries hint')},
    'mesh_zl':             {'type':int,'default':19,'values':(16,17,18,19,20),'hint': _('mesh_zl hint')},
    # Mesh
    'curvature_tol':       {'type':float,'default':2,'hint': _('curvature_tol hint')},
    'apt_curv_tol':        {'type':float,'default':0.5,'hint': _('apt_curv_tol hint')},
    'apt_curv_ext':        {'type':float,'default':0.5,'hint': _('apt_curv_ext hint')},
    'coast_curv_tol':      {'type':float,'default':1,'hint': _('coast_curv_tol hint')},
    'coast_curv_ext':      {'type':float,'default':0.5,'hint': _('coast_curv_ext hint')},
    'limit_tris'    :      {'type':int,  'default':0,'hint': _('limit_tris hint')},
    'hmin':                {'type':float,'default':0,'hint': _('hmin hint')},
    'min_angle':           {'type':float,'default':10,'hint': _('min_angle hint')},
    'sea_smoothing_mode':  {'type':str,  'default':'zero','values':['zero','mean','none'],'hint': _('sea_smoothing_mode hint')},
    'water_smoothing':     {'type':int,  'default':10,'hint': _('water_smoothing hint')},
    'iterate':             {'type':int,  'default':0,'hint': _('iterate hint')},
    # Masks
    'mask_zl':             {'type':int,'default':14,'values':(14,15,16),'hint': _('mask_zl hint')},
    'masks_width':         {'type':list,'default':100,'hint': _('masks_width hint')},
    'masking_mode':        {'type':str,'default':'sand','values':['sand','rocks','3steps'],'hint': _('masking_mode hint')},
    'use_masks_for_inland':{'type':bool,'default':False,'hint': _('use_masks_for_inland hint')},
    'imprint_masks_to_dds':{'type':bool,'default':False,'hint': _('imprint_masks_to_dds hint')},
    'masks_use_DEM_too':   {'type':bool,'default':False,'hint': _('masks_use_DEM_too hint')},
    'masks_custom_extent': {'type':str,'default':'','hint': _('masks_custom_extent hint')},
    # DSF/Imagery
    'default_website':     {'type':str,'default':'','hint':''},
    'default_zl':          {'type':int,'default':16,'hint':''},
    'zone_list':           {'type':list,'default':[],'hint':''},
    'cover_airports_with_highres':{'type':str,'default':'False','values':('False','True','ICAO','Existing'),'hint': _('cover_airports_with_highres hint')},
    'cover_extent':        {'type':float,'default':1,'hint': _('cover_extent hint')},
    'cover_zl':            {'type':int,'default':18,'hint': _('cover_zl hint')},
    'sea_texture_blur':    {'type':float,'default':0,'hint': _('sea_texture_blur hint')},
    'add_low_res_sea_ovl': {'type':bool,'default':False,'hint': _('add_low_res_sea_ovl hint')},
    'experimental_water':  {'type':int,'default':0,'values':(0,1,2,3),'hint': _('experimental_water hint')},
    'ratio_water':         {'type':float,'default':0.25,'hint': _('ratio_water hint')},
    'normal_map_strength': {'type':float,'default':1,'hint': _('normal_map_strength hint')},
    'terrain_casts_shadows':{'type':bool,'default':True,'hint': _('terrain_casts_shadows hint')},
    'overlay_lod':         {'type':float,'default':25000,'hint': _('overlay_lod hint')},
    'use_decal_on_terrain':{'type':bool,'default':False,'hint': _('use_decal_on_terrain hint')},
    # Other
    'custom_dem':          {'type':str,'default':'','hint': _('custom_dem hint')},
    'fill_nodata':         {'type':bool,'default':True,'hint': _('fill_nodata hint')}
}
